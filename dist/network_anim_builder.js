...
var NetworkAnimBuilder = function NetworkAnimBuilder(p) {
    ...
    p.setOnNewConfigurationCallback(function (config) {
        ...
        particles = [];
        for (var i = 0; i < animation.nbParticles; i++) {
            addParticle(particles);
        }
    });
    ...
    function isParticleOutsideWorld(particle) {
        return particle.x > p.getWidth() + animation.maxDistance || particle.x < -animation.maxDistance || particle.y > p.getHeight() + animation.maxDistance || particle.y < -animation.maxDistance;
    }
